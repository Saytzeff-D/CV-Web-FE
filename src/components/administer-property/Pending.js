            ...
            <div>
                <ul className="nav nav-tabs mb-3">
                    <li className="nav-item">
                        <button className="nav-link active border-0 text-dark" data-bs-toggle="tab" data-bs-target="#land">
                        Land
                        </button>
                    </li>
                    <li className="nav-item">
                        <button className="nav-link border-0 text-dark" data-bs-toggle="tab" data-bs-target="#apartment">
                        Apartment
                        </button>
                    </li>                    
                </ul>
                ...
                <div className="tab-content">
                    <div className="tab-pane fade show active" id="land">
                        {
                            isLoading ? (
                                <p className="text-center text-muted">Loading land properties that are pending approval...</p>
                            ) : null
                        }
                        {
                            properties.filter(each=>each.type.toLowerCase() === "land").length === 0 && !isLoading && (
                                <p className="text-center text-muted">No pending land properties.</p>
                            )
                        }
                        {
                            properties.filter(each=>each.type.toLowerCase() === "land").map((p) => (
                    ...
                    <div className="tab-pane fade" id="apartment">
                        {
                            isLoading ? (
                                <p className="text-center text-muted">Loading Apartment properties that are pending approval...</p>
                            ) : null
                        }
                        {
                            properties.filter(each=>each.type.toLowerCase() !== "land").length === 0 && !isLoading && (
                                <p className="text-center text-muted">No pending apartment properties.</p>
                            )
                        }
                        {
                            properties.filter(each=>each.type.toLowerCase() !== "land").map((p) => (
