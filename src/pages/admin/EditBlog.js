...
const EditBlog = () => {
  ...
  const edit = sessionStorage.getItem("editBlog");
  ...
  useEffect(()=>{
      if(edit){
        editor.commands.setContent(edit ? JSON.parse(edit).content : "");
        setImages([JSON.parse(edit).main_photo]);
    }else navigate('/admin/blog-manager')
  }, [])
  ...
  const formik = useFormik({
    initialValues: {
      title: edit ? JSON.parse(edit).title : "",
      subtitle: edit ? JSON.parse(edit).subtitle : "",
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      if (images.length !== 0) {        
        setIsLoading(true);
        axios
          .patch(`${uri}blog/${JSON.parse(edit).id}`, finalData, {
            headers: {
